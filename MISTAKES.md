...
48. **The rotation procedure leaked the key the rotation existed to protect**
Symptom: I told Meet to rotate the API key because it had appeared in a session transcript, then handed him `python3 -c "...print('GRC_API_KEY='+secrets.token_urlsafe(32))"` — which printed the new key straight into the same transcript · Cause: I designed the command around generating a key, not around where the key would end up. The threat was "the secret is visible in a place that can be shared", and the fix reproduced it exactly · Fix: generate directly into `.env` with `re.sub` and print nothing; read it into `TF_VAR_api_key` via command substitution, which never echoes; move it to the browser with `pbcopy` rather than to the screen. Verified afterwards that both exposed keys return 401 and the new one returns 200 · Lesson: a control can be correct and be defeated by how it is operated. "We rotate credentials" and "rotating a credential does not expose it" are separate claims, and `COMPLIANCE.md` can only make the first kind. Also worth recording: the old key was rejected immediately here only because the apply happened to cause a cold start — `_expected_api_key()` is lru_cached per container, so a warm one would have kept honouring the revoked key. Rotation without forcing a cold start is not revocation.

49. **The deployment policy could build the stack and not take it down**
Symptom: `tofu destroy` reported success on 25 of 27 resources and left an S3 bucket and an IAM role behind. Two AccessDenied errors: `s3:DeleteObjectVersion` and `iam:ListInstanceProfilesForRole` · Cause: the policy was written and proved by running `apply`, repeatedly, over six rounds of permission failures (entry 42). Nothing had ever run in the other direction, so two destroy-only permissions were missing and could not have been noticed. `force_destroy` on a versioned bucket deletes every object *version*, which is a different action from deleting an object; and the AWS SDK checks a role for attached instance profiles before deleting it, so removing a role that has never touched EC2 still needs permission to go looking for one · Fix: added both to `docs/aws/deploy-policy.json`, re-ran, 2 destroyed, state empty · Lesson: half of an infrastructure lifecycle was untested, and it happened to be the half that costs money if it fails. A destroy that half-works is worse than one that fails outright — it reports success, leaves resources billing, and the state file agrees with it. Also the third instance of entry 42's lesson in one project: I keep deriving IAM actions from what a service ought to need rather than from what it says it needs.
